...
const EntryPage = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center relative px-3" style={{ backgroundColor: '#001f3f' }}>
      <div className="text-center space-y-6 sm:space-y-8 md:space-y-12 w-full max-w-md sm:max-w-2xl md:max-w-4xl">
        <h1 className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white tracking-wider px-2" style={{ fontFamily: 'Georgia, serif' }}>
          Blankenship
        </h1>
        ...
        <div className="flex gap-1.5 sm:gap-2 md:gap-3 justify-center">
          {code.map((digit, index) => (
            <input
              ...
              onPaste={handlePaste}
              className="w-9 h-11 sm:w-12 sm:h-14 md:w-14 md:h-16 text-center text-lg sm:text-xl md:text-2xl font-semibold bg-white text-gray-900 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition-all"
            />
          ))}
        </div>
        
        <p className="text-white/70 text-xs sm:text-sm tracking-wide px-2">Enter 8-digit access code</p>
      </div>
      ...
      <div className="absolute bottom-4 sm:bottom-8 left-0 right-0 flex justify-center gap-4 sm:gap-6 md:gap-8 px-4">
        ...
        <Dialog>
          <DialogTrigger asChild>
            <button className="flex flex-col items-center gap-1 sm:gap-2 text-white/80 hover:text-white transition-colors group">
              <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-all group-hover:scale-110">
                <Scale className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8" />
              </div>
              <span className="text-[0.65rem] sm:text-xs font-medium">Federal Law</span>
            </button>
          </DialogTrigger>
          <DialogContent className="max-w-2xl max-h-[80vh]">
            <DialogHeader>
