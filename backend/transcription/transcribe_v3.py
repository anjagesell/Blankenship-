#!/usr/bin/env python3
"""
Transcription script - handles dict response format from Whisper API
"""
...
async def transcribe_chunk(api_key, chunk_info, prompt):
    """Transcribe chunk - handles both dict and object responses"""
    ...
    print(f"  Chunk {chunk_info['chunk_num']}...", flush=True)
    ...
    try:
        ...
        # Handle response as dict or object
        if isinstance(response, dict):
            resp_segments = response.get('segments', [])
            resp_text = response.get('text', '')
        else:
            resp_segments = getattr(response, 'segments', []) or []
...
async def main():
    ...
    try:
        ...
        prompt = """Police interrogation. Speakers: Marcella McCombs (female lead interrogator), 
Sergeant Scronce (male interrogator), Zachary Blankenship (defendant). 
Rylie spelled R-Y-L-I-E. Transcribe verbatim."""
